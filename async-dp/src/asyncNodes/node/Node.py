...
class Node:
    ...
    def connect_neighbor(self, neighbor, linger=0):
        """
        Connects given neighbor. Sends HELLO.

        """
        logging.debug("Sending connection request to {}".format(neighbor))
        self.communication.init_connection(neighbor, linger=linger)
    ...
    def connect_neighbors(self):
        ...
        for neighbor in self.my_neighbors:
            if not self.communication.already_connected(neighbor):
                self.connect_neighbor(neighbor, linger=self.linger)
    ...
    def init_dataset_model(self, config, random_seed=97):
        """
        ...
        if "random_seed" in dataset_configs:
    ...
    def instantiate(
        ...
        *args
        ...
        self.init_dataset_model(config)
